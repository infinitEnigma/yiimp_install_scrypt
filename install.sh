...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    # Compil Blocknotify
    cd ~
    git clone https://github.com/infinitEnigma/yiimp-sin.git
...
' | sudo -E tee /etc/yiimp/keys.php >/dev/null 2>&1
    ...
    # Import sql dump
    sudo zcat 2019-06-09-yaamp.sql.gz | sudo mysql --defaults-group-suffix=host1
...
define('"'"'NICEHASH_DEPOSIT'"'"','"'"'3DTqBkT9TMZ6VAyzkK5QJ1GxGzwnR9ngiR'"'"');
define('"'"'NICEHASH_DEPOSIT_AMOUNT'"'"','"'"'0.01'"'"');
$cold_wallet_table = array(
	'"'"'3DTqBkT9TMZ6VAyzkK5QJ1GxGzwnR9ngiR'"'"' => 0.10,
);
